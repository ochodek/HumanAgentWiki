...
COLS = "id, file, category, node_type, title, links, text, meta, updated_at"
...
def brain_search(query: str, k: int = 8, category: str = "", node_type: str = "",
    ...
    Returns ranked notes with provenance, a snippet and `updated` (YYYY-MM-DD)."""
...
def brain_get(title_or_file: str) -> list:
    ...
    conn = connect()
    cur = conn.cursor(row_factory=dict_row)
    cur.execute("SELECT file, category, node_type, title, links, text, meta, updated_at FROM chunks "
