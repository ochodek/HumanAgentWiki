...
def test_partial_index_failure_keeps_existing_chunks_untouched(monkeypatch):
    def fail_prepare(_files):
        raise ValueError("embedding failed")

    monkeypatch.setattr(index, "prepare_files", fail_prepare)
    monkeypatch.setattr(index, "replace_files", lambda *_args: pytest.fail("must not replace"))
    with pytest.raises(ValueError, match="embedding failed"):
        index.reindex_files(None, ["existing.md"])


def test_partial_index_replaces_prepared_chunks_inside_one_transaction(monkeypatch):
    from contextlib import contextmanager
    events = []
    prepared = (["note.md"], [{"text": "new"}], [[1.0]])
    ...
    def replace(cur, *parts):
        assert isinstance(cur, Cursor)
        assert parts == prepared
        events.append("replace")
        return 1

    monkeypatch.setattr(index, "prepare_files", prepare)
    monkeypatch.setattr(index, "replace_files", replace)
    assert index.reindex_files(Cursor(), ["note.md"]) == 1
    assert events == ["prepare", "begin", "replace", "commit"]
